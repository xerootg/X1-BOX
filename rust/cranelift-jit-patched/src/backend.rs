...
pub struct JITModule {
    ...
    // x1-box patch: BTreeMap keyed by start addr; value = (end, FuncId).
    // Upstream stored a Vec and re-sorted on every finalize_definitions().
    // Per-TB finalize then ran an N-log-N sort over every previously-compiled
    // function, costing O(N^2 log N) cumulative and showing up as a 3.5%
    // wall-clock sink + the slump signature on active scenes. BTreeMap
    // gives O(log N) insert + O(log N) lookup with no per-finalize sort.
...
impl JITModule {
    ...
    pub fn finalize_definitions(&mut self) -> ModuleResult<()> {
        ...
        // x1-box patch: code_ranges is now a BTreeMap, naturally sorted.
        // No per-finalize sort needed.
    ...
    pub fn lookup_wasmtime_exception_data<'a>(
        ...
        // x1-box patch: BTreeMap range lookup. Find the largest start <= pc,
        // then check the stored end exceeds pc.
...
impl Module for JITModule {
    ...
    fn define_function_with_control_plane(
        ...
        // x1-box patch: BTreeMap insert keeps code_ranges sorted by construction;
        // no per-finalize O(N log N) sort over all previously-compiled functions.
